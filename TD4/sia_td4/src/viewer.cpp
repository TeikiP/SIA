...
void Viewer::loadImage(const filesystem::path &filename)
{
    ...
    performLayout(mNVGContext);
    m_panel->setPosition(Eigen::Vector2i((mFBSize[0]/mPixelRatio - m_panel->size().x()) / 2, m_resultImage->rows()));
}
...
void renderBlock(Scene* scene, ImageBlock& block)
{
    ...
    uint nbSample = camera->sampleCount();
        ...
        for (int x=0; x<size.x(); ++x) {
            ...
            for(uint i=0; i<nbSample; ++i) {
                for(uint j=0; j<nbSample; ++j) {
                    ...
                    ray.direction = (camF + camX * (2.f * float(x + (i-xx)/float(nbSample) + offset[0])/float(camera->vpWidth())  - 1.f)
                                      - camY * (2.f * float(y + (j-yy)/float(nbSample) + offset[1])/float(camera->vpHeight()) - 1.f)).normalized();
